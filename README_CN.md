...
由于版权原因，请将 `assets/umasso` 替换为你从 Huawei UMA 官方渠道下载且有权使用的可执行程序；当前文件为体验版。如需 aarch64 或其他架构，请替换为对应的官方程序，并相应调整 `scripts/run.sh` 中的架构检查。
...
```text
请按照 uma-sso skill 的 references/install-shell.md 为当前 shell 安装 uma 命令。
```
...
4. 选择合适的账号、`SSH`、密码、“不使用远程客户端”和“本地运维”方式，然后点击登录。
5. 在网络请求中复制以 `sso://` 开头的访问 URL。

## 用法 ⌨️

直接给你的 AI agent 发送 `sso://` 开头的 URL
...
LICENSE                     BSD-3-Clause 许可证正文
NOTICE                      许可范围与 UMA 程序排除声明
```

---

🏵 UMA SSO Skill © Josh Zeng. 采用 [BSD-3-Clause 许可证](LICENSE)发布，`assets/umasso` 除外；详见 [NOTICE](NOTICE)。
